...
export function mapSeatsAeroItem(item, tipo = 'ida') {
    ...
    // Tenta extrair link do primeiro trip (API retorna AvailabilityTrips: null atualmente)
    const firstTrip = Array.isArray(item.AvailabilityTrips) ? item.AvailabilityTrips[0] : null;
    const bookingLinks = firstTrip?.BookingLinks ?? firstTrip?.booking_links ?? [];
    const primaryLink = bookingLinks.find(l => l.primary || l.Primary) ?? bookingLinks[0] ?? null;
    const apiLink = primaryLink?.link ?? primaryLink?.Link ?? primaryLink?.URL ?? primaryLink?.url ?? null;

    // Fallback: URL de busca no seats.aero para este programa/rota/data
    const seatsAeroSource = (item.Source ?? '').toLowerCase();
    const seatsAeroDate = item.Date ?? '';
    const bookingLink = apiLink ?? (
        seatsAeroSource && origin && dest
            ? `https://seats.aero/search?origin=${origin}&destination=${dest}&date=${seatsAeroDate}&source=${seatsAeroSource}`
            : null
    );
