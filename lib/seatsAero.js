...
export async function fetchSeatsAeroAPI(origin, destination, startDate, endDate, userId = null) {
    ...
    if (items.length > 0) {
        // Log primeiro item completo para inspecionar campos disponíveis (booking_links, etc.)
        console.log('[Seats.aero] Primeiro item (raw):', JSON.stringify(items[0], null, 2));
    }
    return items;
}
...
export function mapSeatsAeroItem(item, tipo = 'ida') {
    ...
    // Extrai link de reserva do primeiro trip — tenta PascalCase e snake_case
    const firstTrip = (item.AvailabilityTrips ?? [])[0] ?? null;
    const bookingLinks = firstTrip?.BookingLinks ?? firstTrip?.booking_links ?? [];
    const primaryLink = bookingLinks.find(l => l.primary || l.Primary)
        ?? bookingLinks[0]
        ?? null;
    const bookingLink = primaryLink?.link ?? primaryLink?.Link ?? primaryLink?.URL ?? primaryLink?.url ?? null;
